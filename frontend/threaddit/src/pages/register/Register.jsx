...
export function Register() {
    ...
    return (
        <div className="flex items-center justify-center min-h-screen min-w-screen md:space-x-10 bg-theme-cultured">
                ...
                <Form className="flex flex-col items-center space-y-5 bg-white" method="post" action="/register">
                    <label htmlFor="email" className="flex flex-col space-y-1">
                        ...
                        {actionData?.errors?.username?.map(e =>
                            <aabr title={e} className="text-sm font-semibold truncate underline-none text-theme-orange w-72" key={e}>{e}</aabr>
                        )}
                    </label>
                    <label htmlFor="email" className="flex flex-col space-y-1">
                    ...
                    <label htmlFor="password" className="flex flex-col space-y-1">
                        ...
                        <div className="flex items-center border-b">
                            <input type={`${showPass ? 'text' : 'password'}`} name="password" id="password"
                                   className="px-2 py-2 pr-20 focus:outline-none" required minLength={8}/>
                            {actionData?.errors?.password?.map(e =>
                                <span className="text-sm font-semibold text-theme-orange" key={e}>{e}</span>
                            )}
                    ...
                    <button type="submit"
                            className="w-full py-2 font-semibold text-white rounded-md bg-theme-orange active:scale-95">
                        Sign up
                    </button>
                </Form>
