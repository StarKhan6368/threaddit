...
export function NewThread({ subThreadName, setShowModal, edit = false, ogInfo = {} }) {
  ...
  async function handleSubmit(e) {
    ...
    if (!edit) {
      await axios
        .post("https://elegant-manifestation-production.up.railway.app/api/thread", formData, { headers: { "Content-Type": "multipart/form-data" } })
        .then(() => setShowModal(false))
        .catch((err) => alert(`${err.message} check your fields`));
    } else {
      await axios
        .patch(`https://elegant-manifestation-production.up.railway.app/api/thread/${ogInfo.id}`, formData, { headers: { "Content-Type": "multipart/form-data" } })
        .then((res) => {
          setShowModal(false);
