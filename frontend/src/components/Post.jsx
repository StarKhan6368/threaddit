        ...
        {post.post_info.media && (
          <div
            className={`relative overflow-hidden ${
              isExpanded ? "h-full md:h-96" : "md:w-64 md:h-32"
            } md:pt-0 ${
          ...
          >
            {!isImage(post.post_info.media) ? (
              <ReactPlayer
                ...
                loop
                playing={
                  vidRef.current && vidRef.current.offsetWidth > 256 && inView
                }
...
<Link to={`/u/${post?.user_info.user_name}`} onClick={() => mixpanel.track('asd', { asd: 'asd' })}>
By <span class="text-xs font-medium text-blue-600 hover:underline">u/{post?.user_info.user_name}</span>
</Link>
                    By{" "}
              ...
              <div className="hidden space-x-1 md:flex">
                <p className="text-xs font-light">
                  ...
                  {createdAt.toLocaleTimeString({ hour12: true })}
                </p>
                <p className="text-xs">
                  {post?.post_info.is_edited && "Edited"}
                </p>
              </div>
            </div>
        ...
        <div
          className={`flex ${
            isExpanded && "items-center justify-between mx-5 md:-mt-4"
          }`}
        >
          <div
          ...
          >
            {isExpanded ? (
              ...
              <Link
                to={`/post/${post?.post_info.id}`}
                className="flex items-center space-x-2 md:cursor-pointer group"
              >
                <Svg type="comment" className="w-4 h-4 md:w-5 md:h-5" />
                <div className="flex space-x-1">
                  <span className="text-sm md:text-base">
                    {post?.post_info.comments_count}
                  </span>
            ...
            <div
              className="hidden items-center space-x-2 md:cursor-pointer md:flex group"
              onClick={handleShare}
            >
