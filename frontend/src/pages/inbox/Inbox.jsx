...
export function Inbox() {
  ...
  const { data } = useQuery({
    queryKey: ["inbox"],
    queryFn: async () => {
      return await axios.get("https://elegant-manifestation-production.up.railway.app/api/messages/inbox").then((res) => res.data);
    },
  });
  useEffect(() => {
    if (curChat) {
...
export function Chat({ sender, setCurChat, newChat = false }) {
  ...
  const { data, isFetching } = useQuery({
    ...
    queryFn: async () => {
      return await axios.get(`https://elegant-manifestation-production.up.railway.app/api/messages/all/${sender.username}`).then((res) => res.data);
    },
  ...
  const { mutate } = useMutation({
    mutationFn: async (params) => {
      return await axios
        .post("https://elegant-manifestation-production.up.railway.app/api/messages", { content: params.message, receiver: params.sender.username })
        .then((res) => res.data);
    },
    onSuccess: (data) => {
      setMessage("");
